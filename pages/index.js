...
function CalcTaxTab({ doCalc }) {
  ...
  async function calc() {
    setErr('')
    if (!salary || +salary <= 0) {
      setErr('Укажите зарплату больше нуля')
      return
    }
    setBusy(true)
    setResult(null)
    ...
    <div>
      ...
      <div className="r2">
        <div className="fi">
          ...
          <select className="inp" value={type} onChange={e=>{setType(e.target.value);setResult(null);setErr('')}}>
        ...
        <div className="fi">
          ...
          <input className="inp" type="number" value={salary} onChange={e=>{setSalary(e.target.value);setResult(null);setErr('')}} placeholder="300 000" onKeyDown={e=>e.key==='Enter'&&calc()}/>
        </div>
      </div>

      {err && (
        <div style={{background:'#fef2f2',border:'1.5px solid #fecaca',borderRadius:10,padding:'9px 14px',marginBottom:12,fontSize:12,color:'#dc2626'}}>
          ⚠️ {err}
        </div>
      )}
