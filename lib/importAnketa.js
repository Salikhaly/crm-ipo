...
export function clientsFromList(rows) {
...
function buildClient({ fio, iin, phone, city, officialIncome, monthlyLoad, downPayment, depositAmount, note }) {
  const c = {
    fio: fio || '', iin: iin || '', phone: phone || '',
    source: 'other', stage: 'new_lead',
    tags: ['импорт'],
  }
  if (city)                     c.city = city
