...
// Singleton клиент.
// В dev-режиме Next.js hot-reload пересоздаёт модули, но не очищает globalThis —
// храним клиент там чтобы не плодить соединения при каждом сохранении файла.
export function getSupabase() {
  ...
  // В production модуль загружается один раз — обычный singleton достаточен
  if (!globalThis.__supabaseClient) {
    globalThis.__supabaseClient = createSupabaseClient()
  }
  return globalThis.__supabaseClient
...
export function dbToClient(r) {
  ...
  return {
    ...
    dealStepsDone:       r.deal_steps_done      || [],
    dealStepsNotes:      r.deal_steps_notes     || {},
    comments:            r.comments             || [],
    tasks:               r.tasks                || [],
    savedCalcs:          r.saved_calcs          || [],
    tags:                r.tags                 || [],
    custom:              r.custom               || {},
    credits:             r.credits              || [],
    program:             r.program              || '',
    reassStatus:         r.reass_status         || {},
...
// №5: Нормализация телефона к +7XXXXXXXXXX — поиск работает в любом формате ввода
...
export function clientToDb(c) {
  return {
    ...
    deal_steps_done:       c.dealStepsDone         || [],
    deal_steps_notes:      c.dealStepsNotes        || {},
    comments:              c.comments              || [],
    tasks:                 c.tasks                 || [],
    contracts5y:           c.contracts5y           || {},
    contracts5y_plus:      c.contracts5yPlus       || {},
    // saved_calcs добавляется отдельно (после миграции 008) — см. addSavedCalcs()
  }
}

// Добавляет saved_calcs к строке только если поле есть у объекта.
// Отделено чтобы деплой кода без миграции 008 не ронял запись клиента.
export function addSavedCalcs(row, c) {
  if (c && c.savedCalcs !== undefined) row.saved_calcs = c.savedCalcs || []
  return row
}

// close_reason (миграция 010) — тот же приём, что и addSavedCalcs:
// колонка добавляется отдельно, чтобы код переживал неприменённую миграцию.
export function addCloseReason(row, c) {
  if (c && c.closeReason !== undefined) row.close_reason = c.closeReason || ''
  return row
}

// Необязательные колонки (могут отсутствовать до применения миграций 008/010).
// При ошибке "column ... does not exist" API удаляет колонку из row и повторяет запрос.
// tags (миграция 012) — тот же приём: колонка отдельно, код переживает неприменённую миграцию
export function addTags(row, c) {
  if (c && c.tags !== undefined) row.tags = Array.isArray(c.tags) ? c.tags : []
  return row
}

// custom (миграция 014) — значения доп. полей карточки
export function addCustom(row, c) {
  if (c && c.custom !== undefined) row.custom = (c.custom && typeof c.custom === 'object') ? c.custom : {}
  return row
}

// credits / program / reass_status (миграция 015) — кредитная история из ПКБ,
// ипотечная программа, статусы переуступки
export function addPkbFields(row, c) {
  if (!c) return row
  if (c.credits     !== undefined) row.credits      = Array.isArray(c.credits) ? c.credits : []
  if (c.program     !== undefined) row.program      = c.program || ''
  if (c.reassStatus !== undefined) row.reass_status = (c.reassStatus && typeof c.reassStatus === 'object') ? c.reassStatus : {}
  return row
}

// deal_steps_* — колонки миграции 004; если её пропустили, сохранение клиента
// не должно падать 500 (реальный инцидент). Данные этапов сделки при отсутствии
// колонки просто не пишутся — это допустимая деградация (урок №4).
export const OPTIONAL_COLUMNS = ['saved_calcs', 'close_reason', 'tags', 'custom', 'deal_steps_done', 'deal_steps_notes', 'credits', 'program', 'reass_status']

// Ищет в тексте ошибки Supabase имя необязательной колонки, которой нет в БД
export function findMissingOptionalColumn(error, row) {
  const msg = error?.message || ''
  return OPTIONAL_COLUMNS.find(col => msg.includes(col) && col in row) || null
}
