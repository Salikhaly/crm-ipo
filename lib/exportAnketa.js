...
export function anketaCells(c, { contractLabel = '', managerName = '' } = {}) {
  ...
  // Нагрузка/мес — одной строкой в таблицу активных договоров листа «Кредиты»
...
export const BASE_HEADERS = ['Телефон','Дата','Менеджер','ФИО','ИИН','ПКР',
  'Нагрузка/мес','СРЗП','Цель','Нал. на руках','Депозит Н','Этап','Тег']

// list — клиенты CRM; ctx: { mgrName(id), stageLabel(id) }
export function baseRows(list, ctx = {}) {
  ...
  for (const c of list) {
    rows.push([
      ...
      stageLabel(c.stage), (c.tags || []).join(', '),
    ])
  }
  return rows
}
